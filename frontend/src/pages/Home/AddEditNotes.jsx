...
const AddEditNotes = ({
  ...
  const editNote = async () => {
    try {
      ...
      if (error.data && error.data.response && error.data.response.message) {
        console.log("error in editNote func", error);
  ...
  return (
    ...
    justify-center absolute -top-3 -right-3 hover:bg-slate-500 "
        onClick={onClose}
      >
      ...
      <div className="flex flex-col gap-2 ">
        ...
        <input
          type="text"
          className="txext-2xl text-slate-950 outline-none"
          placeholder="GO to class at 5 "
          value={title}
